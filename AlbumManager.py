					...
					match format_name:
						case 'flac':
							...
							with open(ffmpeg_track_list_file_path, 'w+', encoding='utf-8') as file:
								for track_path in track_paths:
									escaped_path = str(track_path.as_posix()).replace("'", r"'\''")
							...
							total_duration = sum(float(td['format']['duration']) for td in tracks_data)
