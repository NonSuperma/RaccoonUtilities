...
def crop_square_from_1920x1080_media(file_path, output_name) -> None:
    ...
    subprocess.run(f'ffmpeg -i "{file_path}" -vf "crop=1080:1080:420:0" {downloads_path}\\{output_name} -y', shell=True, capture_output=True)
    return None
...
def main():
    ...
    if ss_dymentions == [1920, 1080]:
        ...
        subprocess.run(f'ffmpeg -i "{downloads_path}\\clipboard__square.png" -vf scale=500:500 "{downloads_path}\\clipboard__square500x500.png" -y', shell=True, capture_output=True)
